...
import type {
  ...
  ArtifactHeader,
  ArtifactBody,
} from "@ethan-computer/protocol-types";
...
function toHeader(data: Record<string, unknown>, body: string): ArtifactHeader {
...
function extractExecutionSteps(body: string): string[] {
  const lines = body.split("\n");
  const steps: string[] = [];
  let inSection = false;
...
function extractUserFacts(body: string): Record<string, unknown> {
  const lines = body.split("\n");
...
function toBody(body: string): ArtifactBody {
  return {
